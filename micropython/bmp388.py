...
class DFRobot_BMP388:
  ...
  def get_calib_data(self):
    calib = self.bmp3_get_regs(0x31,21) #读取校准数据
    self.parse_calib_data(calib)
  def uint8_int(self,num):
    if(num>127):
      num = num - 256
    return num
  def parse_calib_data(self,calib):#解析
    temp_var = 0.00390625
  ...
  def set_config(self):
    ...
    self.bmp3_set_sensor_settings(settings_sel) #设置传感器
    self.op_mode = 0x03
  ...
  def compensate_pressure(self,uncomp_pressure,t_lin):
    ...
    partial_out2 = uncomp_pressure *(self.par_p1-0.0001525 +partial_data1+ partial_data2 + partial_data3)
...
if 0:
  ...
  while 1:
    print(bmp388.readTemperature())
    print(bmp388.readPressure())    
    time.sleep(0.5)
else:
  i2c = I2C(scl=Pin(22), sda=Pin(21), freq=100000)
  bmp388 = DFRobot_BMP388_I2C(i2c)
